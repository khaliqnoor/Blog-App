...
const PostPage = ({params}) => {
    ...
    const recentBlogs = posts.filter((post) => post.slug !== slug).slice(0, 3)
    ...
    const handleDelete = (id)=> {
        const finalones = comments.filter((c)=> c.id !== comment.id)
        setComments(finalones)
    }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="mt-16 border-t border-neutral-300 dark:border-neutral-700 pt-10">
        ...
        <button onClick={handleComment} className="mt-3 bg-black dark:bg-white text-white dark:text-black py-2 px-5 rounded-lg font-medium hover:opacity-90 transition">
          Post Comment
        </button>

        {/* Example Comments */}
        {isSignedIn ?   <div className="mt-10 flex flex-col gap-6">
             {comments.map((e) => (
                <div key={e.id} id={e.id} className="border border-neutral-200 dark:border-neutral-700 p-4 rounded-lg">
                    <div className="flex justify-between">
                <p className="text-sm font-semibold">{user.fullName}</p>
                <button onClick={(e)=>handleDelete(e.id)} className="text-red-700 hover:text-red-800 font-semibold cursor-pointer">Delete</button>
                </div>
        ...
        </div> : <div className="text-center font-bold text-red-700">Please sign in to view and post comments</div>
        }
      </div>
